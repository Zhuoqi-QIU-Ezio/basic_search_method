...
class Layout:
    ...
    def save_maze_distances(self, dist_text):
        ...
        for x1 in xrange(self.width):
            for y1 in xrange(self.height):
                if self.walls[x1][y1]: continue
                pos1 = (x1, y1)
                for x2 in xrange(self.width):
                    for y2 in xrange(self.height):
                        if self.walls[x2][y2]: continue
                        self.distance[(pos1, (x2, y2))] = int(dist_text.next().strip())
...
def try_to_load(full_name):
    ...
    try: 
        ...
        with file(full_name[:-4]+".dst") as d_file:
