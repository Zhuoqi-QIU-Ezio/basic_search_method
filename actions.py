...
class Actions:
    ...
    directions_list = directions.items()

    @staticmethod
    def get_legal_actions(pos, walls, other_agent):
        ...
        for dir, vec in Actions.directions_list:
            next_x = x + vec[0]
            if next_x < 0 or next_x == len(walls): continue
            next_y = y + vec[1]
            if next_y < 0 or next_y == len(walls[0]): continue
            if not walls[next_x][next_y] and (next_x, next_y) != other_agent:
                possible.append(dir)
        return possible
