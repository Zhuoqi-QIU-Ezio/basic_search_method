...
class Game:
    ...
    def __init__(self, red_bird_agent, black_bird_agent, display, rules,
        ...
        import cStringIO
        self.agent_output = [cStringIO.StringIO() for agent in self.agents]

    def get_progress(self):
        """ Return the progress of the game (how many yellow birds are left).
            (Game) -> float
        """
        if self.game_over: return 1.0
        else: return self.rules.get_progress(self)
        ...
        while not self.game_over:
            ...
            if 'observation_function' in dir( agent ):
                ...
                if self.catch_exceptions:
                    ...
                    observation = agent.observation_function(self.state.deepcopy())
                self.unmute()
            else:
