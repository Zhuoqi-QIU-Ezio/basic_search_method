...
from game_rules import YELLOW_BIRD_SCORE, TIME_PENALTY
import util
...
class PositionSearchProblem(SearchProblem):
    ...
    def goal_test(self, state):
        ...
        if is_goal: self.visited_goal(state) #For display purposes only
        return is_goal
...
class MultiplePositionSearchProblem(SearchProblem):
    ...
    def __init__(self, state):
        ...
        self.initial_state = (state.get_red_bird_position(), state.get_yellow_birds())
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if next_x >= 0 and next_x < self.width and next_y >= 0 and next_y < self.height and\
                ...
                if next_pos in state[1]:
                    next_yellow_bird = tuple([yb for yb in state[1] if yb != next_pos])
                else:
...
class AdversarialSearchProblem(SearchProblem):
    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            next_pos = (next_x, next_y)
            if next_x >= 0 and next_x < self.width and next_y >= 0 and next_y < self.height and\
                not self.walls[next_x][next_y] and next_pos != other_pos:
                next_score = score
                next_yellow_birds = yellow_birds
                if next_pos in yellow_birds:
                    next_yellow_birds = tuple([yb for yb in yellow_birds if yb != next_pos])
                    next_score += player_yb_score
                if player == 0:
                    succ.append(((1, next_pos, black_pos, next_yellow_birds, next_score, yb_score*0.99), action, 1))
                else:
                    succ.append(((0, red_pos, next_pos, next_yellow_birds, next_score, yb_score*0.99), action, 1))
        if not succ:
            succ.append(((1-player, red_pos, black_pos, yellow_birds, score, yb_score*0.99), Directions.STOP, 1))
        return succ
