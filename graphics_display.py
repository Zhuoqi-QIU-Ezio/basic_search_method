...
BLACK_BIRD_VEC_COLORS = map(color_to_vector, BLACK_BIRD_COLORS)
...
class RedBirdGraphics:
    ...
    def draw_state_objects(self, state):
        ...
        self.agent_images.append((red_bird_pos, self.draw_red_bird(red_bird_pos)))
        black_bird_pos = state.get_black_bird_position()
        if black_bird_pos is not None:
            self.agent_images.append((black_bird_pos, self.draw_black_bird(black_bird_pos)))
    ...
    def swap_images(self, agent_index, new_pos):
        ...
        if agent_index == 0:
            self.agent_images[agent_index] = (new_pos, self.draw_red_bird(new_pos))
        else:
            self.agent_images[agent_index] = (new_pos, self.draw_black_bird(new_pos))
        refresh()

    def update(self, new_state, agent_index):
        prev_pos, prev_image = self.agent_images[agent_index]
        
        if agent_index == 0: agent_pos = new_state.red_bird_position  
        else: agent_pos = new_state.black_bird_position
        ...
        if new_state._yellow_bird_eaten is not None:
            self.remove_yellow_bird(new_state._yellow_bird_eaten, self.yellow_birds)
    ...
    def animate_bird(self, agent_id, bird, prev_bird, image):
        ...
        if self.frame_time > 0.01 or self.frame_time < 0:
            ...
            for i in range(1,int(frames) + 1):
                ...
                if agent_id: self.move_black_bird(pos, image)
                else:        self.move_red_bird(pos, image)
                refresh()
                sleep(abs(self.frame_time) / frames)
        else:
            if agent_id: self.move_black_bird(bird, image)
            else:        self.move_red_bird(bird, image)
    ...
    def draw_yellow_bird(self, yellow_birds):
        yellow_bird_images = [[None for y in xrange(self.height)] for x in xrange(self.width)]
    ...
    def draw_expanded_cells(self, cells):
        ...
        for k, cell in enumerate(cells):
            ...
            cell_color = format_color(*[(n-k) * c * .5 / n + .25 for c in baseColor])
